			...
			if(total > 0){
				...
				do {
					...
					if (oggettiList.isEmpty() && oggettiSearch.getPage() > 0) {
						if(log.isInfoEnabled()) {
							log.info("The request page "+oggettiSearch.getPage()+" was empty."+((oggettiSearch.getPage() > 1)?" Try with page "+(oggettiSearch.getPage() - 1)+".":""));
						}
		...
		ExcelComposer c = new ExcelComposer() {
			...
			protected Object translate(Field field, Object value) {
				if (field.getName().equals("categoria")) {
					...
					try{
						AttributeView av = sysAttributeHandler.getAttribute("QBO001_Categoria", getUserContext().getLanguageForSysAttribute(), getUserContext().getOrganizationForSysSysAttribute(), stringValue, getUserContext());
						if (av != null){
