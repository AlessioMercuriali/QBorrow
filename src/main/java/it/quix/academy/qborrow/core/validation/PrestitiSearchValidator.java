...
public class PrestitiSearchValidator extends QborrowAbstractValidator<PrestitiSearch> {
    ...
    protected void customValidation(UserContext userContext, Set<InvalidConstraint<?>> errors, String propertyPath, PrestitiSearch prestitiSearch,
        ...
        // controllo i range, se non sono nulli, devono essere inseriti in modo corretto: prestitiSearch.getScadenza_prestitoFrom() PRIMA di
        // prestitiSearch.getScadenza_prestitoTo()
        if (prestitiSearch.getScadenza_prestitoFrom() != null && prestitiSearch.getScadenza_prestitoTo() != null) {
            if (prestitiSearch.getScadenza_prestitoFrom().after(prestitiSearch.getScadenza_prestitoTo())) {
                InvalidConstraint<PrestitiSearch> ic1 =
                    new InvalidConstraintImpl<PrestitiSearch>(PrestitiSearch.class, "error.dateToBeforeDateFrom", propertyPath + "scadenza_prestitoFrom",
                        prestitiSearch, prestitiSearch.getScadenza_prestitoFrom());
                InvalidConstraint<PrestitiSearch> ic2 =
                    new InvalidConstraintImpl<PrestitiSearch>(PrestitiSearch.class, "error.dateToBeforeDateFrom", propertyPath + "scadenza_prestitoTo",
                        prestitiSearch, prestitiSearch.getScadenza_prestitoTo());
                errors.add(ic1);
                errors.add(ic2);
            }
        }
