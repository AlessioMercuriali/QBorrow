...
public abstract class PrestitiAbstractDAO extends AbstractJDBCDAO {
	...
	public void create(Prestiti prestiti) throws DAOCreateException {
		...
		try {
			...
			query.append("   (BENEFICIARIO, OGGETTO_PRESTATO, DATA_PRESTITO, SCADENZA_PRESTITO) ").append(EOL);
			...
			super.setParameterDateTime(statement, p++, prestiti.getScadenza_prestito());
			...
			if (numberOfCreatedRecord < 1) {
				String msg = FrameworkStringUtils.concat("An error occurred creating Prestiti on database. No record created. Number of inserted rows: ", numberOfCreatedRecord);
	...
	public void update(Prestiti prestiti) throws DAOStoreException {
		...
		try {
			...
			if (numberOfUpdatedRecord < 1) {
				String msg = FrameworkStringUtils.concat("Error while updating the record of type Prestiti ", prestiti, " on database. Number of updated rows: ", numberOfUpdatedRecord);
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
	...
	protected boolean isDirty(Prestiti oldPrestiti, Prestiti newPrestiti) {
		if (oldPrestiti.getData_prestito() != null && !oldPrestiti.getData_prestito().equals(newPrestiti.getData_prestito())) return true;
		if (oldPrestiti.getData_prestito() == null && newPrestiti.getData_prestito() != null) return true;
		if (oldPrestiti.getScadenza_prestito() != null && !oldPrestiti.getScadenza_prestito().equals(newPrestiti.getScadenza_prestito())) return true;
		if (oldPrestiti.getScadenza_prestito() == null && newPrestiti.getScadenza_prestito() != null) return true;
		if(oldPrestiti.getSoggetti() != null && !oldPrestiti.getSoggetti().equals(newPrestiti.getSoggetti())) return true;
		if(oldPrestiti.getSoggetti() == null && newPrestiti.getSoggetti() != null) return true;
		if(oldPrestiti.getOggetti() != null && !oldPrestiti.getOggetti().equals(newPrestiti.getOggetti())) return true;
		if(oldPrestiti.getOggetti() == null && newPrestiti.getOggetti() != null) return true;
	...
	public static Prestiti cloneWithoutChildren(Prestiti prestiti) {
		if (!prestiti.isJdbc()) {
			throw new ModelJdbcException("The method PrestitiAbstractDAO.cloneWithoutChildren(Prestiti prestiti) can be executed only on jdbc model. The Prestiti pass as parameter isn't jdbc model.");
		}
		...
		newPrestiti.setScadenza_prestito(prestiti.getScadenza_prestito());
	 ...
	 * 
	 * @param soggetti_username - username
	 * @param oggetti_id
	 * @return the object Prestiti
	 * @throws DAOFinderException if no record found with passed params
	 */
	public Prestiti get(String soggetti_username, Integer oggetti_id) throws DAOFinderException {
		Connection connection = null;
		PreparedStatement statement = null;
		ResultSet rs = null;
		try {
			...
			throw new DAOFinderException(FrameworkStringUtils.concat("Cannot find Prestiti on database with [soggetti_username = ",soggetti_username, " oggetti_id = ",oggetti_id, "]"));
			
		} catch (SQLException ex) {
			String msg = FrameworkStringUtils.concat("Error on method get(String soggetti_username, Integer oggetti_id) for Prestiti on database with [soggetti_username = ",soggetti_username, " oggetti_id = ",oggetti_id, "]");
			if (log.isErrorEnabled()) {
				log.error(msg);
			}
			throw new SystemException(msg, ex);
	...
	public Prestiti buildModelFromResultSet(ResultSet rs, QborrowManager qborrowManager) throws SQLException {
		...
		prestiti.setScadenza_prestito(getParameterDate(rs, "scadenza_prestito"));
	 ...
	 */
	public void delete(String soggetti_username, Integer oggetti_id) throws DAODeleteException {
		Connection connection = null;
		PreparedStatement statement = null;
		ResultSet rs = null;
		try {
			...
			if (numberOfDeletedRecord < 1) {
				String msg = FrameworkStringUtils.concat("No record of Prestiti deleted with params [soggetti_username = ",soggetti_username, " oggetti_id = ",oggetti_id,"]");
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
			...
			String msg = FrameworkStringUtils.concat("Error during delete records on Prestiti with params [soggetti_username = ",soggetti_username, " oggetti_id = ",oggetti_id,"]"); 
			if (log.isErrorEnabled()) {
				log.error(msg);
			}
			throw new SystemException(msg, ex);
	...
	public void deletePrestitiListBySoggetti(String soggetti_username) throws DAODeleteException {
		...
		try {
			...
			if (numberOfDeletedRecord < 1) {
				String msg = FrameworkStringUtils.concat("An error occurred delete Prestiti with soggetti_username = ", soggetti_username," on database. No record created. Number of deleted rows: ", numberOfDeletedRecord);
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
				throw new DAODeleteException(msg);
	...
	public void deletePrestitiListByOggetti(Integer oggetti_id) throws DAODeleteException {
		...
		try {
			...
			if (numberOfDeletedRecord < 1) {
				String msg = FrameworkStringUtils.concat("An error occurred delete Prestiti with oggetti_id = ", oggetti_id," on database. No record created. Number of deleted rows: ", numberOfDeletedRecord);
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
				throw new DAODeleteException(msg);
	...
	protected StringBuilder getWhereQuery(Map<Integer, Object> parameters, PrestitiSearch search) {
		...
		if (search.getScadenza_prestitoFrom() != null) {
			whereClause.append("AND scadenza_prestito >= ? ");
			parameters.put(new Integer(p), search.getScadenza_prestitoFrom());
			p++;
		}
		if (search.getScadenza_prestitoTo() != null) {
			whereClause.append("AND scadenza_prestito <= ? ");
			parameters.put(new Integer(p), search.getScadenza_prestitoTo());
			p++;
		} 
