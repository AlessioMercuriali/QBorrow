...
public abstract class OggettiAbstractDAO extends AbstractJDBCDAO {
	...
	public void create(Oggetti oggetti) throws DAOCreateException {
		...
		try {
			...
			super.setParameterDateTime(statement, p++, oggetti.getData_ultima_modifica());
			...
			if (numberOfCreatedRecord < 1) {
				String msg = FrameworkStringUtils.concat("An error occurred creating Oggetti on database. No record created. Number of inserted rows: ", numberOfCreatedRecord);
	...
	public void update(Oggetti oggetti) throws DAOStoreException {
		...
		try {
			...
			if (numberOfUpdatedRecord < 1) {
				String msg = FrameworkStringUtils.concat("Error while updating the record of type Oggetti ", oggetti, " on database. Number of updated rows: ", numberOfUpdatedRecord);
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
	...
	protected boolean isDirty(Oggetti oldOggetti, Oggetti newOggetti) {
		if (oldOggetti.getId() != null && !oldOggetti.getId().equals(newOggetti.getId())) return true;
		if (oldOggetti.getId() == null && newOggetti.getId() != null) return true;
		if (oldOggetti.getTitolo() != null && !oldOggetti.getTitolo().equals(newOggetti.getTitolo())) return true;
		if (oldOggetti.getTitolo() == null && newOggetti.getTitolo() != null) return true;
		if (oldOggetti.getDescrizione() != null && !oldOggetti.getDescrizione().equals(newOggetti.getDescrizione())) return true;
		if (oldOggetti.getDescrizione() == null && newOggetti.getDescrizione() != null) return true;
		if (oldOggetti.getImmagine() != null && !oldOggetti.getImmagine().equals(newOggetti.getImmagine())) return true;
		if (oldOggetti.getImmagine() == null && newOggetti.getImmagine() != null) return true;
		if (oldOggetti.getCategoria() != null && !oldOggetti.getCategoria().equals(newOggetti.getCategoria())) return true;
		if (oldOggetti.getCategoria() == null && newOggetti.getCategoria() != null) return true;
		if (oldOggetti.getData_ultima_modifica() != null && !oldOggetti.getData_ultima_modifica().equals(newOggetti.getData_ultima_modifica())) return true;
		if (oldOggetti.getData_ultima_modifica() == null && newOggetti.getData_ultima_modifica() != null) return true;
		if(oldOggetti.getSoggetti() != null && !oldOggetti.getSoggetti().equals(newOggetti.getSoggetti())) return true;
		if(oldOggetti.getSoggetti() == null && newOggetti.getSoggetti() != null) return true;
	...
	public static Oggetti cloneWithoutChildren(Oggetti oggetti) {
		if (!oggetti.isJdbc()) {
			throw new ModelJdbcException("The method OggettiAbstractDAO.cloneWithoutChildren(Oggetti oggetti) can be executed only on jdbc model. The Oggetti pass as parameter isn't jdbc model.");
		}
	...
	public void deleteOggettiListBySoggetti(String soggetti_username) throws DAODeleteException {
		...
		try {
			...
			if (numberOfDeletedRecord < 1) {
				String msg = FrameworkStringUtils.concat("An error occurred delete Oggetti with soggetti_username = ", soggetti_username," on database. No record created. Number of deleted rows: ", numberOfDeletedRecord);
				if (log.isWarnEnabled()) {
					log.warn(msg);
				}
				throw new DAODeleteException(msg);
